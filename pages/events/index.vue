  ...
  <div class="w-full lg:ps-64">
    <div class="p-4 sm:p-6 space-y-4 sm:space-y-6">
      <DatatablesDataTable
          ...
          { label: 'TANGGAL EVENT', key: 'event_date' },
          { label: 'LOKASI', key: 'location' },
          { label: 'KATEGORI', key: 'category' },
          { label: 'MAX PESERTA', key: 'max_participants' },
...
const fetchData = async () => {
  ...
  try {
    ...
    const response = await fetch(
        `/api/auth/events?page=${currentPage.value}&pagesize=${pageSize.value}`,
        {
          method: "GET",
          credentials: "include",
          headers: {
...
const handleSearchData = async (query: string) => {
  try {
    ...
    isLoading.value = true;
    const response: any = await useFetchApi(`/api/auth/events/search?q=${query}`);
    ...
    console.error("Gagal mencari data events:", e);
  } finally {
    isLoading.value = false;
  }
};
